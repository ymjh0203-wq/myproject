# ============================================================
# app.py  —  벤치마킹 소싱 웹 화면 (Streamlit)
# ------------------------------------------------------------
# 실행:
#   벤치마킹소싱_실행.bat  (더블클릭)
#   또는  .venv\Scripts\streamlit.exe run app.py
#
# 흐름: 기준 상품(URL/이미지) → 대표이미지 → 타오바오 이미지검색 →
#       유사 후보 N개 → 각 추출 → 카드로 보기 → 선택 저장(products_raw)
#
# 주의: 검색 시 "타오바오 크롬 창"이 따로 뜹니다. 로그인/캡차가 보이면
#       그 창에서 직접 처리하세요. 결과가 뜨면 이 화면이 자동으로 이어받습니다.
#       (최초 1회 로그인은 타오바오_로그인_설정.bat 로 해두면 편합니다.)
# ============================================================
...
st.set_page_config(page_title="벤치마킹 소싱", page_icon=":material/search:", layout="wide")


def run_in_thread(fn, *args, **kwargs):
    """
    Playwright(sync)를 Streamlit 스크립트 스레드가 아닌 '새 스레드'에서 돌립니다.
    (sync API가 이벤트 루프 없는 스레드에서 안전하게 동작하도록)
    """
...
with st.form("search_form"):
    ...
    url = st.text_input(
        "한국 마켓 상품 URL",
        placeholder="https://smartstore.naver.com/... 또는 쿠팡 상품 주소",
    )
    uploaded = st.file_uploader(
        "또는 상품 이미지 업로드", type=["jpg", "jpeg", "png", "webp"]
    )
